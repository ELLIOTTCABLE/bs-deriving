...
type a1 = int        [@@ppx_deriving show]
type a2 = int32      [@@ppx_deriving show]
type a3 = int64      [@@ppx_deriving show]
type a4 = nativeint  [@@ppx_deriving show]
type a5 = float      [@@ppx_deriving show]
type a6 = bool       [@@ppx_deriving show]
type a7 = char       [@@ppx_deriving show]
type a8 = string     [@@ppx_deriving show]
type a9 = bytes      [@@ppx_deriving show]
type r  = int ref    [@@ppx_deriving show]
type r2 = int Pervasives.ref [@@ocaml.warning "-3"] [@@ppx_deriving show]
type r3 = int Pervasives.ref ref [@@ocaml.warning "-3"] [@@ppx_deriving show]
type l  = int list   [@@ppx_deriving show]
type a  = int array  [@@ppx_deriving show]
type o  = int option [@@ppx_deriving show]
type f  = int -> int [@@ppx_deriving show]
type y  = int lazy_t [@@ppx_deriving show]
let test_alias ctxt =
...
type v = Foo | Bar of int * string | Baz of string [@@ppx_deriving show]
...
type rv = RFoo | RBar of { x: int; y: string } | RBaz of { z: string } [@@ppx_deriving show]
...
type 'a pt = { v : 'a } [@@ppx_deriving show]
...
module M' = struct
  type t = M.t = A [@@ppx_deriving show]
end
...
module rec RecFoo : sig
  type ('a,'b) t = ('b, 'a) RecBar.t [@@ppx_deriving show]
end = struct
  type ('a,'b) t = ('b,'a) RecBar.t [@@ppx_deriving show]
end
and RecBar : sig
  type ('b, 'a) t = 'b * 'a [@@ppx_deriving show]
end = struct
  type ('b,'a) t = 'b * 'a [@@ppx_deriving show]
end
...
#if OCAML_VERSION >= (4, 03, 0)
type i_has_result = I_has of (bool, string) result [@@ppx_deriving show]
...
type i_has_result_result = I_has of (bool, string) Result.result [@@ppx_deriving show]
...
type no_full    = NoFull   of int [@@ppx_deriving show { with_path = false }]
type with_full  = WithFull of int [@@ppx_deriving show { with_path = true  }]
module WithFull = struct
  type t = A of int [@@ppx_deriving show ]
end
